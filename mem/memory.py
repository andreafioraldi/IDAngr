...
class SimSymbolicIdaMemory(SimMemory): #pylint:disable=abstract-method
    ...
    def _split_segment(addr, segments):
        s_idx = SimSymbolicIdaMemory._get_segment_index(addr, segments)
        ...
        segments.pop(s_idx)
        SimSymbolicIdaMemory._create_segment(segment['start'], size_prev, segment['options'], s_idx, segments)
        SimSymbolicIdaMemory._create_segment(addr, size_next, [{"idx": opt["idx"] + size_prev}
    ...
    def _get_segments(addrs, size):
        ...
        for addr in addrs:
            if addr < highest:
                idx = SimSymbolicIdaMemory._split_segment(addr, segments)
                SimSymbolicIdaMemory._create_segment(highest, addr + size - highest, [], len(segments), segments)
                SimSymbolicIdaMemory._add_segments_overlap(idx, addr, segments)
            else:
                SimSymbolicIdaMemory._create_segment(addr, size, [{'idx': 0}], len(segments), segments)
            highest = addr + size
        return segments
    ...
    #
    # Things that are actually handled by SimIdaMemory
    #
    ...
    def changed_bytes(self, other):
        ...
        :param other:   The other :class:`SimSymbolicIdaMemory`.
